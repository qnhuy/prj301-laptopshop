 ...
 * @author NCPC
 */
@WebServlet(name = "UpdateStatusOrder", urlPatterns = {"/admin/update"})
public class UpdateStatusOrder extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            statusId = Integer.parseInt(action);
            OrderDAO orderDAO = new OrderDAO();
            orderDAO.updateStatusOrder(oId, statusId);
            if (action.equals("2")) {
                session.setAttribute("message", "Xác nhận đơn hàng thành công");
            } else if (action.equals("3")) {
               session.setAttribute("message", "Xác nhận giao hàng thành công"); 
            } else {
